...
export interface McpSearchFilters { experience_min?: number; experience_max?: number; posted_after?: string; posted_before?: string; limit?: number; }
export interface McpSearchResponse { results: JobPost[]; servers: Record<string, {status:string; result_count:number; source?:string}>; }
export async function searchMcp(server_ids: string[], keywords: string[], location?: string, remote?: boolean, filters: McpSearchFilters = {}): Promise<McpSearchResponse> { const r = await fetch(`${API_BASE_URL}/mcp/search`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({server_ids, keywords, location, remote, ...filters})}); if (!r.ok) throw new Error('MCP search failed'); const data = await r.json(); return {results:(data.results || []).map(mapJob),servers:data.servers || {}}; }
